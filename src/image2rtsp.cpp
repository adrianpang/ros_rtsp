...
void Image2RTSPNodelet::onInit() {
    ...
    string pipeline_tail =  " ! rtph264pay name=pay0 pt=96 )"; // Gets completed based on rosparams below
  ...
  rtsp_server = rtsp_server_create(port);
    ...
    for(XmlRpc::XmlRpcValue::ValueStruct::const_iterator it = streams.begin(); it != streams.end(); ++it)
    {
        ...
        if (stream["type"]=="cam")
        {
            pipeline = "( " + static_cast<std::string>(stream["source"]) + " ! videoconvert ! vaapih264enc bitrate=" + bitrate + pipeline_tail;
        ...
        else if (stream["type"]=="topic")
        {
            ...
            * so we know to stop subscribing when no-one is connected. */
            ...
            pipeline = "( appsrc name=imagesrc do-timestamp=true is-live=true ! videoscale ! " + caps + " ! videoconvert ! vaapih264enc bitrate=" + bitrate + pipeline_tail;
